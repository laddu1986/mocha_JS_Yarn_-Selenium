...
describe('Template Service -> Template Properties', () => {
    ...
    it('enablePropertyRule() can enable numberRange property rule', async () => {
        let response = await properties.modifyProperty(templateData, 'enablePropertyRule', Constants.TemplateProperties.Rules.NumberRange);
        expect(response.status.code).to.equal(0);
        joi.assert(response.response, schemas.renamePropertySchema());
    });
    it('changePropertyRule() can change numberRange property rule', async () => {
        let response = await properties.changePropertyRule(templateData, 'integerRule', Constants.TemplateProperties.Rules.NumberRange);
        expect(response.status.code).to.equal(0);
        joi.assert(response.response, schemas.renamePropertySchema());
    });
    ...
    it('disablePropertyRule() can disable numberRange property rule ', async () => {
        let response = await properties.modifyProperty(templateData, 'disablePropertyRule', Constants.TemplateProperties.Rules.NumberRange);
        expect(response.status.code).to.equal(0);
        joi.assert(response.response, schemas.renamePropertySchema());
    });
